...
const PlayerBanner = () => {
    ...
    return (
        <View className="bg-white">
            <Image className="w-96 h-96 absolute -right-40 -top-20 opacity-5" source={ImageAssets[data[0].currentTeam.id]} />

            <View className="flex flex-row items-center py-5 mt-10 px-5">
                <Image
                ...
                <View className="ml-5">
                    <Text style={{ fontFamily: "Poppins_700Bold" }} className="text-xl text-neutral-90 break-normal leading-7">
                        {data[0].firstName} {data[0].lastName}
                    </Text>

                    <View className="flex flex-row items-center">
                        <Text className="text-center text-xs text-neutral-600">
                            {data[0].currentTeam.name} • #{data[0].primaryNumber}
                        </Text>
                    </View>
                </View>
            </View>
